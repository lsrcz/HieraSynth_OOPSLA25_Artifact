...
USE_VL_MASK=""
NAME_POSTFIX=
SKETCH_SELECTOR=fixed_cmp_inferred_7
NUM_OF_RUNS=3
...
NAME=Min128.Fixed${NAME_POSTFIX}.${NUM_OF_CORES}
if [[ -d logs/${NAME} ]]; then
  mkdir -p logs/"${NAME}".old
  mv logs/"${NAME}"/* logs/"${NAME}".old
fi
hyperfine -i -r${NUM_OF_RUNS} --export-json "results/${NAME}.json" \
  "./scripts/target/min_max_128.sh -j ${NUM_OF_CORES} --ir-func-name Min128 --sketch-selector=${SKETCH_SELECTOR} --log-prog-name ${NAME} --target-cost 15 ${USE_VL_MASK}"
