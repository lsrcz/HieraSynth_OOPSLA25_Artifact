...
USE_VL_MASK=""
NAME_POSTFIX=
SKETCH_SELECTOR=sub_prog_inferred_7
NUM_OF_RUNS=3
...
NAME=Lt128${NAME_POSTFIX}.${NUM_OF_CORES}
if [[ -d logs/${NAME} ]]; then
  mkdir -p logs/"${NAME}".old
  mv logs/"${NAME}"/* logs/"${NAME}".old
fi
hyperfine -i -r${NUM_OF_RUNS} --export-json "results/${NAME}.json" \
  "./scripts/target/lt128.sh -j ${NUM_OF_CORES} --ir-func-name Lt128 --sketch-selector=${SKETCH_SELECTOR} --log-prog-name ${NAME} ${SCHEDULER_TIMEOUT} --target-cost 11 ${USE_VL_MASK}"
