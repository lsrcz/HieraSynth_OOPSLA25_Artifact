...
for _ in $(seq 1 2); do
  ./scripts/target/odd_even_blocks.sh -j 1 --scheduler-timeout 1 >/dev/null 2>/dev/null
done
...
for func in $list; do
  ...
  hyperfine -i -r${NUM_OF_RUNS} --export-json "results/${NAME}.json" \
    "./scripts/target/odd_even_blocks.sh -j ${NUM_OF_CORES} --ir-func-name ${func} --log-prog-name ${NAME} ${USE_VL_MASK} ${USE_OPTIMAL_SKETCH}"
