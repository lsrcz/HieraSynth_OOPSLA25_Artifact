...
for _ in $(seq 1 2); do
  ./scripts/target/concat.sh -j 1 --scheduler-timeout 1 >/dev/null 2>/dev/null
done

list="ConcatLowerLower ConcatLowerUpper ConcatUpperUpper ConcatUpperLower"
...
for func in $list; do
  ...
  hyperfine -i -r${NUM_OF_RUNS} --export-json "results/${NAME}.json" \
    "./scripts/target/concat.sh -j ${NUM_OF_CORES} --ir-func-name ${func} --log-prog-name ${NAME} ${USE_VL_MASK} ${USE_OPTIMAL_SKETCH}"
