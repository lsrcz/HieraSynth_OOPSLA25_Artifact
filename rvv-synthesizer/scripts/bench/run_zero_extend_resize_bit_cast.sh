...
for i in $(seq 1 2); do
  ./scripts/target/zero_extend_resize_bit_cast.sh -j 1 --scheduler-timeout 1 >/dev/null 2>/dev/null
done

targs=('' '-a"vuint8m2_t,vuint8m1_t"')
postfixs=('' '.Ext')

for i in $(seq 1 2); do
  NAME=ZeroExtendResizeBitCast${postfixs[i]}${NAME_POSTFIX}.${NUM_OF_CORES}
  if [[ -d logs/${NAME} ]]; then
    mkdir -p logs/"${NAME}".old
    mv logs/"${NAME}"/* logs/"${NAME}".old
  fi
  hyperfine -i -r${NUM_OF_RUNS} --export-json "results/${NAME}.json" \
    "./scripts/target/zero_extend_resize_bit_cast.sh -j ${NUM_OF_CORES} ${targs[i]} --log-prog-name ${NAME} ${USE_VL_MASK} ${USE_OPTIMAL_SKETCH}"
